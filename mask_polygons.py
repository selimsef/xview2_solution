...
def mask_polygons_together(size, shapes):
    ...
    for u in shapes:
        poly, damage = shapes[u]
        fillPoly(mask_img, [poly], (damage, damage, damage))
...
def get_feature_info(feature, post_feature):
    """
    ...
    class_mapping = {
        ...
        'major-damage': 3,
        'destroyed': 4,
        'un-classified': 255 #ignored
    }
    props = {}
    damage_type = {}
    for feat in post_feature['features']['xy']:
        damage_type[feat['properties']['uid']] = class_mapping[feat['properties']['subtype']]

    for feat in feature['features']['xy']:
        feat_shape = wkt.loads(feat['wkt'])
        coords = list(mapping(feat_shape)['coordinates'][0])
        props[feat['properties']['uid']] = (np.array(coords, np.int32)), damage_type[feat['properties']['uid']]
...
def mask_chips(json_path, images_directory, output_directory, single_file, border):
    ...
    for j in tqdm([j for j in jsons if j.endswith('json')],
        ...
        chip_post_json = read_json(j_full_path.replace("_pre", "_post"))
        ...
        polys = get_feature_info(chip_json, chip_post_json)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input',
                        default="/home/selim/datasets/xview/",
